...
const outputDir = join(root, "test-results", "showcase");
...
await rm(join(root, "data", "showcase.sqlite"), { force: true });
...
const server = spawn("pnpm", ["dev"], {
  ...
  env: {
    ...
    DATABASE_URL: `file:${join(root, "data", "showcase.sqlite")}`,
    GOOGLE_PLACES_ENABLED: "false",
    PERFORMER_PASSPHRASE: passphrase,
    SESSION_SECRET: "openreveal-showcase-local-secret-change-me",
...
try {
  ...
  await runShowcaseFlow(performerPage, receiverPage);
  ...
  const performerWebm = join(outputDir, "openreveal-showcase-performer.webm");
  const receiverWebm = join(outputDir, "openreveal-showcase-audience-phone.webm");
  ...
  if (ffmpeg) {
    const performerMp4 = join(outputDir, "openreveal-showcase-performer.mp4");
    const receiverMp4 = join(outputDir, "openreveal-showcase-audience-phone.mp4");
    combinedMp4 = join(outputDir, "openreveal-showcase-combined.mp4");
  ...
  console.log("Showcase QA recording passed.");
...
async function runShowcaseFlow(performerPage, receiverPage) {
...
async function writeQaSummary({ combinedMp4, error, outputs, passed }) {
  const lines = [
    ...
    "- Custom text reveal renders as centered full-screen text.",
    "- Celebrity reveal renders with a Google Search fallback link.",
